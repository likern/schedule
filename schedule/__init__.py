...
class Scheduler(object):
    ...
    def after(self, after_time=1):
        job = Job(after=after_time)
        self.jobs.append(job)
        return job
    ...
    def join(self, obj):
        ...
        if isinstance(obj, list):
            if not obj:
                return self
            obj.sort(key=attrgetter('run_after'))
            jobs = list()
            for scheduler in obj:
                if not isinstance(scheduler, Scheduler):
                    raise TypeError('Not expected type: ${0}'.format(type(scheduler)))
                jobs.extend(scheduler.jobs)
            self.jobs += jobs
        else:
            raise TypeError('Not expected type: ${0}'.format(type(obj)))
        return self
...
class Job(object):
    ...
    * a quantity of `time units` defined by `interval`
    ...
    #     def __init__(self, after = 0, interval = 1):
    #         self.after = after
    #         self.interval = interval

    def __init__(self, interval, after, after_unit):
        self.interval = interval  # pause interval * unit between runs
        self.after = after
        self.after_unit = after_unit
    ...
    def interval(self, interval):
        if interval < 0:
            raise ValueError('Not expected value: ${0}'.format(interval))
        self.interval = interval
    ...
    def after(self, after):
        if after < 0:
            raise ValueError('Not expected value: ${0}'.format(after))
        self.after = after
...
class AfterJob(object):
    ...
    * a quantity of `time units` defined by `interval`
    ...
    def __init__(self, interval = 1, after = 0):
        self.interval = interval  # pause interval * unit between runs
        self.after = after
        self.job_func = None  # the job job_func to run
        self.unit = None  # time units, e.g. 'minutes', 'hours', ...
        self.after_unit = None
        self.at_time = None  # optional time at which this job runs
        self.last_run = None  # datetime of the last run
        self.next_run = None  # datetime of the next run
        self.period = None  # timedelta between runs, only valid for
    ...
    def every(self, interval=1):
        ...
        :return: An empty job
        """
        job = Job(after=self.after, interval=interval, after_unit=self.unit)
        self.jobs.append(job)
        return job
...
def after(obj):
    return default_scheduler.after(obj)
...
def _convert_if_time_is_supported(obj):
    if isinstance(obj, datetime.datetime):
        ...
        return obj
    raise TypeError('Not supported time type: ${0}'.format(type(obj)))
